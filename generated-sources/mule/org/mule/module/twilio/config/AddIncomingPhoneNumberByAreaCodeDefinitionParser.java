...
import org.mule.module.twilio.processors.AddIncomingPhoneNumberByAreaCodeMessageProcessor;
...
public class AddIncomingPhoneNumberByAreaCodeDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(AddIncomingPhoneNumberByAreaCodeDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(AddIncomingPhoneNumberByAreaCodeMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [add-incoming-phone-number-by-area-code] within the connector [twilio] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [add-incoming-phone-number-by-area-code] within the connector [twilio] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("addIncomingPhoneNumberByAreaCode");
