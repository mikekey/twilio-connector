...
import org.mule.module.twilio.processors.UpdateIncomingPhoneNumbersMessageProcessor;
...
public class UpdateIncomingPhoneNumbersDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(UpdateIncomingPhoneNumbersDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(UpdateIncomingPhoneNumbersMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [update-incoming-phone-numbers] within the connector [twilio] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [update-incoming-phone-numbers] within the connector [twilio] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("updateIncomingPhoneNumbers");
        ...
        parseProperty(builder, element, "accountSidDestination", "accountSidDestination");
